    ...
    <div class="content-wrapper transition-all ease-in-out duration-700">
        <div class="flex flex-col gap-y-2">
            <div class="bg-white p-5 mb-4 flex items-center justify-between rounded-sm shadow-md">
                <h1 class="lg:text-2xl text-xl font-medium mt-1 text-gray-700">Create Promo Code</h1>
            ...
            <div class="bg-white p-6 rounded-sm shadow-md">
                <form action="{{ route('admin_promo_codes_store') }}" method="POST">
                    @csrf
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="number" name="min_transaction" value="{{ old('min_transaction') }}"
                                step="0.01" min="0" class="w-full border border-gray-300 rounded-lg px-4 py-2">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="per_user_limit" value="{{ old('per_user_limit', 1) }}"
                                min="1" required class="w-full border border-gray-300 rounded-lg px-4 py-2">
                        </div>
                        ...
                        <div>
                            ...
                            <select name="applicable_to" class="w-full border border-gray-300 rounded-lg px-4 py-2">
                                <option value="all" {{ old('applicable_to') == 'all' ? 'selected' : '' }}>All Bookings
                                </option>
                                <option value="destinations"
                                    {{ old('applicable_to') == 'destinations' ? 'selected' : '' }}>Destinations Only
                                </option>
                                <option value="hotels" {{ old('applicable_to') == 'hotels' ? 'selected' : '' }}>Hotels Only
                                </option>
                            </select>
                        </div>
                        ...
                        <div class="flex items-center">
                            <input type="checkbox" name="is_active" id="is_active" value="1"
                                {{ old('is_active') ? 'checked' : 'checked' }} class="w-4 h-4 text-blue-600 rounded">
