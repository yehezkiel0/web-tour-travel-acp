<div id="itinerary" class="tab-pane md:col-span-2 mb-14">
    ...
    <p class="text-gray-3 text-[10px] mb-7">*Changes may occur according to on-site conditions without reducing
        the planned attractions</p>

    @if (!empty($itineraries) && is_array($itineraries) && count($itineraries) > 0)
        <div class="relative space-y-8">
            {{-- Main vertical dashed line --}}
            <div
                class="absolute top-5 left-6 h-[calc(100%-2rem)] w-px border-l-2 border-dashed border-secondary -translate-x-1/2">
            </div>

            {{-- Itinerary items --}}
            @foreach ($itineraries as $itinerary)
                @if (is_array($itinerary))
                    {{-- New format: array with title, duration, description --}}
                    <div class="relative flex flex-row items-start space-x-10">
                        ...
                        <div class="flex-1">
                            <h3 class="text-lg font-semibold text-secondary-600">
                                {{ $itinerary['title'] ?? 'Day ' . $loop->iteration }}</h3>

                            @if (!empty($itinerary['duration']))
                                <p class="text-sm font-medium text-blue-600 mt-1">{{ $itinerary['duration'] }}</p>
                            @endif

                            <p class="mt-2 text-gray-600 leading-relaxed">{{ $itinerary['description'] ?? '' }}</p>

                            @if (!empty($itinerary['alternative']))
                                <p class="mt-2 text-sm italic text-gray-500">Alternative:
                                    {{ $itinerary['alternative'] }}</p>
                            @endif
                        </div>
                    </div>
                @else
                    ...
                    <div class="relative flex flex-row items-start space-x-10">
                        ...
                        <div class="flex-1">
                            <h3 class="text-lg font-semibold text-secondary-600">Day {{ $loop->iteration }}</h3>
                            <p class="mt-2 text-gray-600">{{ $itinerary }}</p>
                        </div>
                    </div>
                @endif
            @endforeach
    ...
    @else
        <div class="text-gray-500 italic">No itinerary information available.</div>
    @endif
</div>
