<section class="footer lg:py-16 border-t border-[#E0E0E0]">
    <div class="container w-[90%] md:w-[80%] lg:w-[90%] xl:max-w-7xl mx-auto py-8">
        <div>
            <img src="{{ asset('images/icon/Logo.svg') }}/" alt="Logo-Footer" class="mb-8" />
            <div class="grid grid-cols-2 md:grid-cols-5 gap-10">
                <div class="col-span-1 md:col-span-2">
                    <h4 class="text-[#333333] text-normal font-medium px-0 md:px-5">
                        <span class="font-bold">ACP Travel & Tours is</span> a trusted travel agency offering
                        comprehensive
                        tourism
                        services to
                        explore South
                        Korea.
                    </h4>
                </div>
                <div class="text-[#333333]">
                    <h3 class="mb-6 font-bold text-lg">For Beginners</h3>
                    <ul class="space-y-2">
                        <li>
                            <a href="#" class="font-normal hover:underline">New Account</a>
                        </li>
                        <li>
                            <a href="#" class="font-normal hover:underline">Start Booking Travel</a>
                        </li>
                        <li>
                            <a href="#" class="font-normal hover:underline">Start Booking Hotels</a>
                        </li>
                    </ul>
                </div>
                <div class="text-[#333333]">
                    <h3 class="mb-6 font-bold text-lg">Explore Us</h3>
                    <ul class="space-y-2">
                        <li>
                            <a href="#" class="font-normal hover:underline">Our Careers</a>
                        </li>
                        <li>
                            <a href="#" class="font-normal hover:underline">Privacy & Policy</a>
                        </li>
                        <li>
                            <a href="#" class="font-normal hover:underline">Terms & Condition</a>
                        </li>
                    </ul>
                </div>
                <div class="text-[#333333]">
                    <h3 class="mb-6 font-bold text-lg">Connect Us</h3>
                    <ul class="space-y-2">
                        <li>
                            <a href="#" class="font-normal hover:underline">support&#64;acptours.id</a>
                        </li>
                        <li>
                            <a href="#" class="font-normal hover:underline">021 - 2208 - 1996</a>
                        </li>
                        <li>
                            <a href="#" class="font-normal hover:underline">Staycation, Kemang, Jakarta</a>
                        </li>
                    </ul>
                </div>
            </div>
        </div>
    </div>
    <div class="container w-[90%] md:w-[80%] lg:w-[90%] xl:max-w-7xl mx-auto pt-12 px-4">
        <div class="flex flex-col-reverse gap-y-4 md:flex-row md:justify-between items-center">
            <p class="text-[#333333] font-normal">
                &copy; 2019 &bull; All rights reserved &bull; ACP Tours
            </p>
            <div class="flex flex-row gap-x-2">
                <a href="#"
                    class="rounded-full border border-[#333333] border-opacity-50 p-2 hover:border-opacity-100">
                    <img src="{{ asset('images/icon/linkedin.svg') }}" alt="">
                </a>
                <a href="#"
                    class="rounded-full border border-[#333333] border-opacity-50 p-2 hover:border-opacity-100">
                    <img src="{{ asset('images/icon/facebook-footer.svg') }}" alt="">
                </a>
                <a href="#"
                    class="rounded-full border border-[#333333] border-opacity-50 p-2 hover:border-opacity-100">
                    <img src="{{ asset('images/icon/instagram.svg') }}" alt="">
                </a>
                <a href="#"
                    class="rounded-full border border-[#333333] border-opacity-50 p-2 hover:border-opacity-100">
                    <img src="{{ asset('images/icon/twitter.svg') }}" alt="">
                </a>
            </div>
        </div>
    </div>
</section>
